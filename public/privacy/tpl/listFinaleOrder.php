...
$objectsAtt = json_decode($_POST['hiddenAtt']);
...
$titleType = (isset($type) && $type == 'oralOrder') ? 'Liste des oraux par ordre de mérite' : 'Liste finale par ordre de mérite';

$titleType_ar = (isset($type) && $type == 'oralOrder') ? 'لائحة الاختبار الشفوي حسب الاستحقاق' : 'التنيجة النهائية للمترشحين الناجحين حسب الاستحقاق';
//var_dump($objects);
//die(); 
...
$contentsT = '';
$contentsT .= '<h2 align="center">قائمة المترشحين المدرجين بلائحة الانتظار حسب الاستحقاق</h2><br />';
$pdf->writeHTML($contentsT);

 $pdf->Ln();

 $text2 = '<table width="100%" border="0" cellpadding="2" cellspacing="0">
    
      <tr>
      <td width="5%" align="center"  bgcolor="#dddddd" > N° Ordre</td>
      <td width="25%" align="center" bgcolor="#dddddd" >N° CIN</td>
      <td width="40%" align="center" bgcolor="#dddddd" >Nom et Prénom</td>
      <td width="30%" align="center" bgcolor="#dddddd" >الإسم والنسب</td>
 
      </tr>';
  $contentTable2 = null;$i = 0;$ngColor = "#ffffff";
  foreach($objectsAtt as $row) {
            $i++;
           ...
           $contentTable2 .= '<tr bgcolor="'.$ngColor.'">
            <td align="center" >'.$i.'</td>
            <td align="center" >'.$row->cin.'</td>
            <td align="center">'.strtoupper($row->nom).' '.strtoupper($row->prenom).'</td>
            <td align="center">'.$row->prenom_ar.' '.$row->nom_ar.'</td>
