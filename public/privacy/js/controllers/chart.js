    ...
    , function($scope,$window, $rootScope, $http, $location,API_URL,toaster,$timeout) {
      ...
      var loadCharts = function(){
       ...
       success(function(data, status, headers, config) {
         ...
         $scope.datac = data.nbre_inscrits_by_search;

         $scope.datasc = data.nbre_inscrits_by_device;

         $scope.datamonths = data.post_by_month;

         $scope.datacountry = data.datacountry;

         $scope.datagloab = data.data_glob;

        //   $scope.nbre_total_formation = data.nbre_total_formation[0].data;
        $scope.nbre_candidat = data.nbre_total_candidat;

        $scope.nbre_users = data.nbre_total_candidat;

                   console.info(data.nbre_inscrits_by_search);  
